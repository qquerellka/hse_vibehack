...
export function ActivityOverview() {
  ...
  if (isError || !health) {
    return <Card>API недоступен. Проверьте запуск backend и контейнера `api`.</Card>
  }

  return (
    <div className="space-y-5">
      <Card className="rounded-[24px] bg-ink p-5 text-paper shadow-none">
        <p className="font-mono text-xs uppercase tracking-[0.22em] text-paper/60">API status</p>
        <div className="mt-4 flex items-center gap-3">
          <HeartPulse className="h-5 w-5 text-paper" />
          <div>
            <p className="text-lg text-paper">{health.status}</p>
            <p className="text-sm text-paper/70">{health.message}</p>
          </div>
        </div>
      </Card>
      ...
      <Card className="rounded-[24px] bg-fog/80 p-5 shadow-none">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl text-ink">Текущий сценарий</h2>
        </div>
        <p className="mt-3 text-sm leading-7 text-muted">
          Интерфейс не использует локальные заглушки: поиск, скачивание, оценка и обзор идут через реальные маршруты backend.
        </p>
      </Card>
      ...
      <Card className="rounded-[24px] bg-white/65 p-5 shadow-none">
        ...
        <p className="mt-3 text-sm leading-7 text-muted">
          {selectedArticleId
            ? `Готова к запуску pipeline для arXiv ID ${selectedArticleId}.`
            : 'Статья ещё не выбрана. Начните с поиска слева.'}
        </p>
      </Card>
    </div>
  )
