...
```env
DEBUG=True
DATABASE_URL=postgresql://postgres:postgres@db:5432/vaiboton
OPENROUTER_API_KEY=
...
1. `db`
   PostgreSQL 15 в контейнере `vaiboton_db`, наружу проброшен порт `5433`.

2. `api`
   Backend в контейнере `vaiboton_api`, запускается командой:
...
3. `frontend`
   Vite dev server в контейнере `vaiboton_frontend`, запускается командой:
